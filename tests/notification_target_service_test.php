...
try{
    ...
    ntTest('shared page grants and roleless targets follow customer visibility policy',function()use($service,$orderId,$restrictedUserId){$r=$service->resolve(['target_type'=>'order','target_id'=>$orderId],$restrictedUserId,['ChinaEmployee']);ntAssert(!empty($r['available']),'shared order page grant was ignored');$r=$service->resolve(['target_type'=>'order','target_id'=>$orderId],$restrictedUserId,[]);ntAssert(empty($r['available']),'roleless target was exposed');});

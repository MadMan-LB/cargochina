...
set FAILED=0
for %%f in (upload_test search_test smoke_test draft_order_builder_test hs_code_catalog_test user_test lifecycle_test financial_test expense_test consolidation_test item_capture_test suppliers_contact_test tracking_push_idempotency_test tracking_push_retry_test phase2_integration_test production_hardening_test regression_receive_variance_test) do (
  echo === %%f ===
  "%PHP%" "%~dp0tests\%%f.php"

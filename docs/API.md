...
- `POST /draft-orders/import` — Multipart preview import using `file` (`.xlsx`, `.xls`, `.csv`, `.cv`). Parses normalized header names instead of fixed column positions, so reordered columns such as `Materials`, `Express Number`, `Supplier`, `Supplier Name`, `Photo`, `SKU / Item Code`, `English Item Name`, `Chinese Item Name`, `Cartons`, `Pieces/Carton`, prices, CBM, weight, HS code, and custom-design fields still map correctly. Optional columns may be missing; exported subtotal/grand-total rows are ignored; the response returns `meta.rows_imported`, `meta.rows_skipped`, `meta.skipped_rows`, `meta.blank_rows_skipped`, `meta.missing_optional_columns`, image counts, warnings, and timing. It returns a builder-shaped payload and does not save an order until the user reviews and saves the draft form. Supplier sections can be marked with a `Supplier:` row and the supplier name in the next cell; the first `Express Number` entered in a supplier section carries down to later blank express-number rows until the next supplier section. XLSX embedded images are extracted from worksheet drawings, matched first by the detected `Photo` column and row overlap, stored into `uploads/`, and attached to the matching item photo field. Repeated description columns are preserved as multiple descriptions. Explicit `Factory Price` / `Customer Price` columns map to those form fields; a bare exported `UNIT PRICE` maps to customer price.

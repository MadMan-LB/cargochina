      ...
      <section class="portal-hero p-4 p-md-5 mb-4">
        <div class="d-flex flex-column flex-md-row justify-content-between gap-3 align-items-md-end">
          <div>
            ...
            <p class="mb-0 opacity-75">Code: <?= htmlspecialchars($customer['customer_code']) ?>. This one-time link has now been activated for this visit.</p>
          </div>
      ...
      <div class="row g-4">
        <?php foreach ($orders as $order): ?>
          ...
          $badgeClass = match ($order['status']) {
            'AwaitingCustomerConfirmation' => 'bg-warning text-dark',
            'Confirmed', 'ReadyForConsolidation', 'AssignedToContainer', 'FinalizedAndPushedToTracking', 'Shipped' => 'bg-success',
            'CustomerDeclined' => 'bg-danger',
            default => 'bg-primary',
          };
          ?>
          <div class="col-12">
                ...
                <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 mb-3">
                  <div>
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                      ...
                      <span class="badge <?= $badgeClass ?>"><?= htmlspecialchars($statusLabels[$order['status']] ?? $order['status']) ?></span>
                    </div>
                    <div class="text-muted small">Supplier: <?= htmlspecialchars($order['supplier_name'] ?: 'Multiple / not specified') ?></div>
                  </div>
                  <div class="row g-2 text-sm-end">
                    <div class="col-auto">
                      <div class="small text-muted">Created</div>
                      <div class="fw-semibold"><?= htmlspecialchars(formatPortalDate($order['created_at'])) ?></div>
                    </div>
                    <div class="col-auto">
                      <div class="small text-muted">Expected Ready</div>
                      <div class="fw-semibold"><?= htmlspecialchars(formatPortalDate($order['expected_ready_date'])) ?></div>
                    </div>
                    <div class="col-auto">
                ...
                <div class="row g-4">
                  <div class="col-12 col-lg-7">
                    ...
                    <div class="timeline-line">
                      <div class="mb-3">
                        ...
                        <div class="text-muted small ms-4"><?= htmlspecialchars(formatPortalDate($order['created_at'], 'Y-m-d H:i')) ?></div>
                      </div>
                      <div class="mb-3">
                        <span class="timeline-dot"></span>
                        <strong>Expected ready date</strong>
                        <div class="text-muted small ms-4"><?= htmlspecialchars(formatPortalDate($order['expected_ready_date'])) ?></div>
                      </div>
                      <?php if (!empty($order['warehouse_received_at'])): ?>
                        <div class="mb-3">
                          <span class="timeline-dot"></span>
                          <strong>Warehouse receipt recorded</strong>
                          <div class="text-muted small ms-4"><?= htmlspecialchars(formatPortalDate($order['warehouse_received_at'], 'Y-m-d H:i')) ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($order['declined_at'])): ?>
                        <div class="mb-0">
                          ...
                          <div class="text-muted small ms-4"><?= htmlspecialchars(formatPortalDate($order['declined_at'], 'Y-m-d H:i')) ?></div>
                          <?php if (!empty($order['decline_reason'])): ?>
                            <div class="small ms-4 text-danger">Reason: <?= htmlspecialchars($order['decline_reason']) ?></div>
                          <?php endif; ?>
                        </div>
                      <?php elseif (!empty($order['confirmed_at'])): ?>
                        <div class="mb-0">
                          ...
                          <div class="text-muted small ms-4"><?= htmlspecialchars(formatPortalDate($order['confirmed_at'], 'Y-m-d H:i')) ?></div>
                        </div>
                      <?php elseif ($order['status'] === 'AwaitingCustomerConfirmation'): ?>
                        <div class="mb-0">
                          <span class="timeline-dot"></span>
                          <strong>Waiting for your confirmation</strong>
                          <div class="text-muted small ms-4">Please review the warehouse measurements below.</div>
                        </div>
                      <?php endif; ?>
                    </div>
                  </div>
                  ...
                  <div class="col-12 col-lg-5">
                    ...
                    <div class="row g-2 mb-3">
                      <div class="col-4">
                        <div class="border rounded-3 p-3 bg-light h-100">
                          ...
                          <div class="fw-semibold"><?= htmlspecialchars($order['actual_cbm'] !== null ? number_format((float) $order['actual_cbm'], 4) : '—') ?></div>
                        </div>
                      </div>
                      <div class="col-4">
                        <div class="border rounded-3 p-3 bg-light h-100">
                          <div class="text-muted small">Actual Weight</div>
                          <div class="fw-semibold"><?= htmlspecialchars($order['actual_weight'] !== null ? number_format((float) $order['actual_weight'], 2) . ' kg' : '—') ?></div>
                        </div>
                      </div>
                      <div class="col-4">
                        <div class="border rounded-3 p-3 bg-light h-100">
                          <div class="text-muted small">Cartons</div>
                          <div class="fw-semibold"><?= htmlspecialchars($order['actual_cartons'] !== null ? (string) $order['actual_cartons'] : '—') ?></div>
                        </div>
                      </div>
                    </div>

                    <div class="small text-muted mb-3">Condition: <?= htmlspecialchars($order['receipt_condition'] ?: '—') ?></div>
                    ...
                    <?php if ($order['status'] === 'AwaitingCustomerConfirmation' && !empty($order['confirmation_token'])): ?>
                      ...
                      <a class="btn btn-primary w-100" href="/cargochina/confirm.php?token=<?= urlencode($order['confirmation_token']) ?>">Review Measurements And Confirm / Decline</a>
                    <?php elseif (!empty($order['confirmed_at'])): ?>
                      <div class="alert alert-success py-2 small mb-0">This order has already been confirmed.</div>
                    <?php elseif (!empty($order['declined_at'])): ?>
                      <div class="alert alert-danger py-2 small mb-0">This order was declined and sent back to the team for review.</div>
                    <?php else: ?>
                      <div class="alert alert-light py-2 small mb-0">No customer action is currently required for this order.</div>
                    <?php endif; ?>
                  </div>
                </div>
              </div>

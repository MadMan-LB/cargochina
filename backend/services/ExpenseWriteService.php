    ...
    public static function save(PDO $pdo,string $method,?int $id,array $input,int $userId): array
    {
        ...
        try {
            ...
            if($method==='DELETE'){
                ...
                if($new['order_id']&&$new['container_id']){$s=$pdo->prepare('SELECT 1 FROM shipment_draft_orders sdo JOIN shipment_drafts sd ON sd.id=sdo.shipment_draft_id WHERE sdo.order_id=? AND sd.container_id=?');$s->execute([$new['order_id'],$new['container_id']]);if(!$s->fetchColumn())jsonError('Expense order does not belong to the selected container',422);}

...
return function (string $method, ?string $id, ?string $action, array $input) {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            if($containerId){$s=$pdo->prepare('SELECT id FROM containers WHERE id=? FOR UPDATE');$s->execute([$containerId]);if(!$s->fetchColumn())jsonError('Container not found',404);$s=$pdo->prepare('SELECT o.id FROM orders o JOIN shipment_draft_orders sdo ON sdo.order_id=o.id JOIN shipment_drafts sd ON sd.deleted_at IS NULL AND sd.id=sdo.shipment_draft_id WHERE sd.container_id=? AND o.customer_id=?'.($orderId?' AND o.id=?':'').' LIMIT 1');$s->execute($orderId?[$containerId,$customerId,$orderId]:[$containerId,$customerId]);if(!$s->fetchColumn())jsonError('Container has no matching customer cargo',422);}

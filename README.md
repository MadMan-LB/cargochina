...
Orders, Receiving, Warehouse Stock, and Draft an Order support current-page checkbox selection. One selected record downloads one workbook; multiple selected records download a ZIP containing one workbook per authorized record. The backend reloads and authorizes every selected ID and limits one batch to 100 records.

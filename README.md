...
The shared Excel service also performs a final canonical lookup by trusted order-item/product IDs when an upstream export payload omits image paths or all supplied paths are stale. SuperAdmins can check a live record under **Diagnostics > Excel Image Health**; the result reports candidate counts, safe failure categories, upload/temp/GD/ZIP health, the image-pipeline version, and source fingerprints without exposing stored paths. After deploying Excel-image changes, deploy the related handlers and service together and restart PHP-FPM (or clear OPcache) so production does not run a mixed release.

...
## 2026-06-28 - Operational Access, Receiving Performance, and Training Reset

- Broadened operational access so Orders, Receiving, Draft an Order, confirmations, safe customer lookup, products read/search, countries read, and related submit/approve/receive/confirm actions are available to operational roles (`ChinaAdmin`, `ChinaEmployee`, `LebanonAdmin`, `WarehouseStaff`, `ContainersStaff`, `FieldStaff`, `SuperAdmin`) through backend RBAC, page guards, sidebar visibility, dashboard task cards, and API router checks.
- Kept customer-owner visibility rules scoped to the full customer management surface (`customers.php` and full customer CRUD/list/detail APIs). Operational selectors now use safe customer lookup/search responses that expose minimal selection fields only.
- Optimized large-order Receiving by batching item fetches server-side, avoiding N+1 queue/search item queries, making search collation safer for mixed encodings, and rendering large item tables in frontend chunks with "show more" instead of freezing the page.
...
- Optimized `backend/services/ReceivingExcelImportService.php`:
  ...
  - If the template has no customer and no customer is selected, direct intake now uses a controlled fallback customer named `Direct Warehouse Intake` instead of blocking the import. The preview warns the user, and the fallback customer is reused or created once during final import with an audit entry.
  - Suppliers referenced by code/name in a valid direct intake are reused when found and created as minimal supplier records when missing.
...
- Validation performed: PHP lint on `receiving.php`, `backend/api/handlers/receiving.php`, and `backend/services/ReceivingExcelImportService.php`; JS syntax check on `frontend/js/receiving.js`; DB-backed preview test with the sample procurement template; no-customer fallback preview test verified rows are warnings instead of errors; rollback-only direct commit test verified 1 order, 2 order items, and 1 warehouse receipt would be created.

## 2026-06-30 - Customer Page Visibility and Customer Creation

- Updated Customers page visibility so `customers` is treated as an operational page for sidebar/direct page access. This avoids production cases where stale saved sidebar JSON hid the Customers link even when admins had checked it for a role.
- Updated customer visibility filtering so legacy/unassigned customer rows (`customers.created_by` NULL or 0) are visible to non-admin operational users. Without this, older imported/customers-created-before-audit rows could not appear for users like Cherry even when customer visibility exceptions were configured.
- Updated `customers.create` so all operational roles can add customers from the Customers page and through the backend API. Import remains controlled separately by `customers.import`.
...
## 2026-07-20 - Master XLSX, Filter Integrity, and Bulk Downloads
